...
class Button : public BaseElement, public DisableableElement {
    protected:
        ...
        bool active;
    public:
        Button(float x, float y, float width, float height, Font *font, std::string  text, uint32_t color, uint32_t hoverColor, uint32_t textColor, uint32_t hoverTextColor, uint32_t borderColor, std::function<void(Button*)> onClick) : BaseElement(), x(x), y(y), width(width), height(height), font(font), text(std::move(text)), color(color), hoverColor(hoverColor), textColor(textColor), hoverTextColor(hoverTextColor), borderColor(borderColor), onClickFn(std::move(onClick)), active(true) {}
        Button(float x, float y, float width, float height, Font *font, std::string  text, uint32_t color, uint32_t hoverColor, uint32_t textColor, uint32_t borderColor,  std::function<void(Button*)> onClick) : BaseElement(), x(x), y(y), width(width), height(height), font(font), text(std::move(text)), color(color), hoverColor(hoverColor), textColor(textColor), hoverTextColor(textColor), borderColor(borderColor), onClickFn(onClick), active(true) {}
        void setOnClick(std::function<void(Button*)> onClick) { this->onClickFn = onClick; }
        void setPos(float x, float y, float width, float height) { this->x = x; this->y = y; this->width = width; this->height = height; }
