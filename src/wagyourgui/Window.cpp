...
Window::Window(const std::string& title, int width, int height, GLFWSession* parent) :
        handle(glfwCreateWindow(width, height, title.c_str(), nullptr, nullptr)), width(width), height(height),
        parent(parent) {
    if (handle == nullptr) {
    ...
    glfwSetKeyCallback(
            handle, [](GLFWwindow* window, int key, int scancode, int action, int mods) {
                auto* thisRef = (Window*) glfwGetWindowUserPointer(window);
    ...
    glfwSetScrollCallback(
            handle, [](GLFWwindow* window, double xOffset, double yOffset) {
                auto* thisRef = (Window*) glfwGetWindowUserPointer(window);
