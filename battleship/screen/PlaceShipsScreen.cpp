...
void PlaceShipsScreen::init(Window *window) {
    ...
    elements.push_back(std::make_shared<Button>(height, height - 30, controlRoom, 20, parent->font, "Undo", 0, 0xFF7F7F7F, 0xFFFFFFFF, 0xFF000000, [=] (Button *button) {
...
void PlaceShipsScreen::onRender(float mouseX, float mouseY) {
    ...
    DrawableHelper::drawCenteredString(parent->font, Battleship::SHIP_NAMES[shipsPlaced], controlRoom, ts + 30, 0xFFFFFFFF);
    glPopMatrix();
}
